 ...
 */
const updateBasicCooldown = async (phoneNumber) => {
  const options = {
  ...
  console.log(
    `Global last notif date updated with current date for user ${phoneNumber}.`
  );
};
 ...
 */
const updateDetailedCooldown = async (phoneNumber, stores) => {
  ...
  await dynamo.update(options).promise();
  console.log(
    `Detailed last notif date updated with current date for user ${phoneNumber}.`
  );
};
